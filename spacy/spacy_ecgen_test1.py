...
# def helper(alltokens, token, strings, f, dicts):
#     for i in range(len(dicts)):
#         if strings[i] not in dicts[i]:
#             dicts[i][strings[i]] = [token.f[i]]
#         else:
#             dicts[i][strings[i]].append(token.function[i])
#         alltokens[token.text].append(dicts[i])

def build_doc_dicts(doc):
    #Total dictionary of tokens first (if we need all tokens for example)
    count = 1
    alltokens = {}
    #strings for all dictionaries
    ...
    functions = ["has_vector", "vector_norm", "is_oov", "lemma_", "tag_", "shape_", "dep_"]
    ...
    for token in doc:
        if token.text not in alltokens:
            ...
            # helper(alltokens, token, strings, functions, dicts)
            ...
            if DEPS not in deps:
                ...
                deps[DEPS].append(token.dep_)
            alltokens[token.text].append(deps)
        # if token.text not in isoov:
        #     isoov[token.text] = [token.is_oov]
        #
        # else:
        #     isoov[token.text].append(token.vector_norm)
        #
        # if token.text not in lemma:
        #     lemma[token.text] = [token.lemma_]
        # else:
        #     lemma[token.text].append(token.lemma_)
